...
public class TomeGui extends SimpleGui {
   ...
   public void buildProfileGui(ServerPlayerEntity player){
      ...
      GuiElementBuilder head = new GuiElementBuilder(Items.PLAYER_HEAD).setSkullOwner(gameProfile,player.getWorld().getServer());
      ...
      for(int i = 0; i < 4; i++){
         for(int j = 0; j < 7; j++){
            if(k < pageItems.size()){
               ...
               try{
                  playerGameProf = player.getWorld().getServer().getUserCache().getByUuid(playerId).orElseThrow();
                  playerItem = new GuiElementBuilder(Items.PLAYER_HEAD).setSkullOwner(playerGameProf,player.getWorld().getServer());
