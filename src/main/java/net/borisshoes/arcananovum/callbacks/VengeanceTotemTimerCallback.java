...
public class VengeanceTotemTimerCallback extends TickTimerCallback {
   ...
   public void onTimer(){
      try{
         ServerPlayerEntity player1 = player.getWorld().getServer().getPlayerManager().getPlayer(player.getUuid());
         if(player1 == null){
            if(attacker != null){
               if(avenged){
                  BorisLib.addLoginCallback(new XPLoginCallback(player.getWorld().getServer(),player.getUuidAsString(),ArcanaConfig.getInt(ArcanaRegistry.TOTEM_OF_VENGEANCE_SURVIVE))); // Give XP
               }else{
