...
public class CommandRegisterCallback {
   public static void registerCommands(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment){
      dispatcher.register(literal("arcana")
            .then(literal("create").requires(CommandManager.requirePermissionLevel(2))
            ...
            .then(literal("cache").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::cacheCommand))
            ...
            .then(literal("blocks").executes(ArcanaCommands::placedBlocks)
                  .then(argument("player",player()).requires(CommandManager.requirePermissionLevel(2))
                        .executes(context -> ArcanaCommands.placedBlocks(context,getPlayer(context,"player")))))
            .then(literal("uuids").requires(CommandManager.requirePermissionLevel(2))
                  .then(argument("player",player()).executes(context -> ArcanaCommands.uuidCommand(context,getPlayer(context,"player")))))
            .then(literal("enhance").requires(CommandManager.requirePermissionLevel(2))
            ...
            .then(literal("xp").requires(CommandManager.requirePermissionLevel(2))
                  .then(literal("add")
                        .then(argument("targets", players())
            ...
            .then(literal("skillpoints").requires(CommandManager.requirePermissionLevel(2))
                  .then(literal("add")
                        .then(argument("targets", players())
                              .then(argument("amount", integer())
            ...
            .then(literal("achievement").requires(CommandManager.requirePermissionLevel(2))
                  .then(literal("grant")
            ...
            .then(literal("research").requires(CommandManager.requirePermissionLevel(2))
                  .then(literal("grant")
            ...
            .then(literal("augment").requires(CommandManager.requirePermissionLevel(2))
                  .then(literal("apply")
            ...
            .then(literal("boss")
                  .then(literal("start").requires(CommandManager.requirePermissionLevel(2))
                        .then(literal("dragon").executes(ArcanaCommands::startDragonBoss)))
                  .then(literal("resetAbilities").requires(CommandManager.requirePermissionLevel(2))
                        .then(argument("doAbility",bool()).executes(context -> ArcanaCommands.bossResetAbilities(context,getBool(context,"doAbility")))))
                  .then(literal("forceLairAction").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::bossForceLairAction))
                  .then(literal("abort").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::abortBoss))
                  .then(literal("clean").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::cleanBoss))
                  .then(literal("status").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::bossStatus))
                  .then(literal("forcePlayerCount").requires(CommandManager.requirePermissionLevel(2))
                        .then(argument("players", integer()).executes(ctx -> ArcanaCommands.bossForcePlayerCount(ctx, getInteger(ctx, "players")))))
                  .then(literal("announce").requires(CommandManager.requirePermissionLevel(2))
                  ...
                  .then(literal("begin").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::beginBoss))
                  .then(literal("teleport")
                        ...
                        .then(literal("all").requires(CommandManager.requirePermissionLevel(2))
      ...
      if(DEV_MODE){
         dispatcher.register(literal("arcana")
               .then(literal("test").requires(CommandManager.requirePermissionLevel(2))
               ...
               .then(literal("getbookdata").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::getBookData))
               .then(literal("getitemdata").requires(CommandManager.requirePermissionLevel(2))
                     .then(argument("name", string()).executes(ctx -> ArcanaCommands.getItemData(ctx, getString(ctx, "name")))))
               .then(literal("makerecipe").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::makeCraftingRecipe))
               .then(literal("boss")
                     .then(literal("test").requires(CommandManager.requirePermissionLevel(2)).executes(ArcanaCommands::testBoss)))
               .then(literal("item").requires(CommandManager.requirePermissionLevel(2))
                     .then(literal("name")
                           .then(literal("set")
